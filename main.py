...
# ========== Транслитерация RU->QWERTY (как в этапах 1-2) ==========
RU_TO_QWERTY = {
...
# ========== Утиль для прав/размеров ==========
def perms_to_string(mode: Optional[int], is_dir: bool) -> str:
    if mode is None:
        # дефолт для VFS: drwxr-xr-x / -rw-r--r--
        if is_dir:
            return 'drwxr-xr-x'
        else:
            return '-rw-r--r--'
    # если режим есть — используем как раньше
...
class IFs:
    ...
    def lstat(self, path: str) -> Tuple[bool, Optional[int], int, float, str]:
        """
        Возвращает кортеж:
        (is_dir, mode_or_None, size_bytes, mtime_epoch, display_name)
        """
        ...

    def exists(self, path: str) -> bool: ...

# ========== Реальная ФС (fallback, как раньше) ==========
class OsFs(IFs):
    def abspath(self, cwd: str, path: str) -> str:
        if not path or path == "~":
...
class VfsNode:
    def __init__(self, name: str, is_dir: bool):
        self.name = name
        self.is_dir = is_dir
        self.children: Dict[str, 'VfsNode'] = {}  # для директорий
        self.content: bytes = b''                 # для файлов
        self.mode: Optional[int] = None
        self.mtime: float = time.time()

class MemoryVfs(IFs):
    """
    Виртуальная ФС целиком в памяти.
    Путь — POSIX-стиль, корень '/'.
    CSV-формат (ожидаемые столбцы по заголовку):
        path,type,data_b64,mode,mtime
    - path: абсолютный или относительный POSIX-путь (например: /, /etc, dir/file.txt)
    - type: dir|file
    - data_b64: для file — содержимое в base64 (может быть пустым), для dir — пусто
    - mode: необязательный int (восьмеричный можно через 0o644), иначе пусто
    - mtime: необязательный float epoch сек, иначе текущее
    Вложенность обеспечивается путями ("/a/b/c.txt").
    """
    def __init__(self):
        self.root = VfsNode('/', True)

    # ---- путь нормализуем и разбиваем
    ...
    def _ensure_dir(self, path: str) -> VfsNode:
        ...
        for part in path.strip('/').split('/'):
            ...
            if nxt is None:
                ...
                # конфликт типов: файл по пути директории — делаем директорию, контент теряем (но такого в валидном CSV быть не должно)
                nxt.is_dir = True
                nxt.content = b''
            node = nxt
        return node
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if t == 'dir':
                    ...
                    # метаданные (если даны)
                    d.mtime = float(mtime_raw) if mtime_raw else d.mtime
                    if mode_raw:
                        try:
                            d.mode = int(mode_raw, 0)  # поддержка 0o755
                        except ValueError:
                            d.mode = None
                    continue

                if t == 'file':
                    # убедимся, что родительские директории присутствуют
    ...
    # ---- IFs интерфейс ----
    def abspath(self, cwd: str, path: str) -> str:
        # ~ трактуем как корень VFS или как /home/user ? Для простоты: ~ == /
        if not path or path == "~":
            return '/'
        if path.startswith('~'):
            # ~user не поддерживаем, просто в корень
            path = path.replace('~', '/', 1)
        if path.startswith('/'):
...
# ========== Вспомогалки для отображения путей ==========
...
def shorten_home_vfs(path: str) -> str:
    # В VFS «домом» считаем корень '/', ради дружелюбного вида
...
class ShellEmulatorGUI:
    def __init__(self, root, fs: IFs, vfs_mode: bool, startup_script: Optional[str], args_debug: str):
        ...
        # Отладка
        self.println("=== Shell Emulator (Stage 3: VFS) ===")
        ...
        self.println("Commands: ls [-a] [-l] [path], cd [path], pwd, exit")
    ...
    def _make_prompt(self) -> str:
        path_display = (self.cwd if not self.vfs_mode else (self.cwd if self.cwd != '/' else '~'))
        if not self.vfs_mode:
    ...
    def _process_line(self, line: str):
        ...
        cmd, *args = parts
        if cmd == "exit":
            self.cmd_exit(); return
        if cmd == "pwd":
            self.cmd_pwd(args); return
        if cmd == "cd":
            self.cmd_cd(args); return
        if cmd == "ls":
            self.cmd_ls(args); return
    ...
    def cmd_ls(self, args: List[str]):
        ...
        for i, p in enumerate(paths):
            ...
            if isdir:
                ...
                for it in items:
                    if not show_all and it.startswith('.'):
                        continue
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Shell Emulator (Stage 3: VFS in-memory from CSV)")
...
def init_fs(vfs_csv: Optional[str]) -> Tuple[IFs, bool, List[str]]:
    logs = []
    if vfs_csv:
        try:
